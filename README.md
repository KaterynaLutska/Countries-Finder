...
Це невеликий додаток пошуку даних про країну по її частковому або повному
імені. 
Використано [Rest Countries API](https://restcountries.eu/), а саме
...
conditions). Тому на обробник події застосувується підхід `debounce` і
робити HTTP-запит через `500мс` після того, як користувач перестав вводити
текст. Використовється npm-пакет
...
зробити запит більш специфічним. Для сповіщень використовується плагін
